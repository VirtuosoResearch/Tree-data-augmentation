...
from torch_geometric.loader import DataLoader
from tawt import get_task_weights_gradients_multi, get_task_weights_gradients_multi_hessian
...
def train_dro(model, device, loader, optimizer, group_weights, weight_lr=1e-2):
    model.train()
    num_groups = len(group_weights)
    group_losses = torch.zeros_like(group_weights, device=device)
    group_samples = torch.zeros_like(group_weights, device=device)
    for step, batch in enumerate(loader):
        ...
        group_losses[group_id] += torch.sum(loss_mat).detach()
        group_samples[group_id] += len(loss_mat)
    ...
    '''
    group_losses = group_losses/group_samples
    exp_ratio = torch.exp(weight_lr * group_losses)
...
def train_bilevel(model, device, loader, optimizer, group_weights, task_to_train_dataloaders,
                 weight_lr=1, collect_gradient_step=1, update_step=50, test_loader = None):
    model.train()
    num_groups = len(group_weights)
    for step, batch in enumerate(loader):
        assert type(batch) == dict
        ...
        if (step+1) % update_step == 0:
            task_weights_gradients = get_task_weights_gradients_multi_hessian(
        ...
        for epoch in range(1, args.epochs+1):
            if args.train_bilevel:
                group_weights = train_bilevel(model, device, train_loader, optimizer, group_weights, task_to_train_dataloaders,
                              args.weight_lr, args.collect_gradient_step, update_step=args.update_step, test_loader=test_loader)
            elif args.train_dro:
