...
def main(args):
    ...
    while cur_depth <= args.max_depth and len(potential_idxes) != 0:
        ...
        command = "python train.py --config configs/config_supervised_cifar100.json --model WideResNet28_10\
                    --augmentation_names {} --augmentation_ratios {} \
                    --augmentation_probs {} --tree_idxes {} --runs 1\
                    --device {} --save_name {} --epochs {} --data_frac {}".format( 
                ...
                args.device, "training", args.epochs, args.data_frac
        )
        print(command)
        os.system(command)
        load_model_dir  = "supervised_training_{}_{}/".format(
            "WideResNet28_10", 
            "_".join(
                ...
                for prob in tmp_prob_list:
                    ...
                    if cur_val is None:
                        command = "python train.py --config configs/config_supervised_cifar100.json --model WideResNet28_10\
                                    --augmentation_names {} --augmentation_ratios {} \
                                    --augmentation_probs {} --tree_idxes {} --runs 1\
                                    --device {} --save_name {} --epochs {} --data_frac {} --load_model_dir {}".format( 
                                augmentation_names, # augmentation_names,
                                augmentation_ratios, # augmentation_ratios,
                                augmentation_probs, # augmentation_probs,
                                tree_idxes, # tree_idxes,
                                args.device, args.save_name, 0, args.data_frac, load_model_dir
                        )
                        print(command)
                        os.system(command)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--eval_metric', type=str, default="val_accuracy", help='eval metric')
