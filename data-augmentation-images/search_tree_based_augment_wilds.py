...
def main(args):
    ...
    while cur_depth <= args.max_depth and len(potential_idxes) != 0:
        ...
        command = "python train_wilds.py --config configs/wild_config.json\
                                ...
                                args.device,  args.epochs, args.batch_size, "training")
        print(command)
        os.system(command)
        load_model_dir  = "iwildcam_ResNet50_{}_{}/".format(
            args.group_id
            "_".join(
                ...
                for prob in tmp_prob_list:
                    ...
                    if cur_val is None:
                        command = "python train_wilds.py --config configs/wild_config.json\
                                    ...
                                    --augmentation_probs {} \
                                    --tree_idxes {} \
                                    --n_gpu 1 --device {} --epochs {} --batch_size {} --save_name {} --load_model_dir {}".format( 
                                args.dataset, args.group_id,
                                augmentation_names, # augmentation_names,
                                augmentation_ratios, # augmentation_ratios,
                                augmentation_probs, # augmentation_probs,
                                tree_idxes, # tree_idxes,
                                args.device, 0, args.batch_size, args.save_name, load_model_dir)
                        print(command)
                        os.system(command)
